...
def index(request):
    ...
    visitor_cookie_handler(request)       #call the helper function to handle the cookies
...
def visitor_cookie_handler(request):
    ...
    if (datetime.now() - last_visit_time).days > 0:     #if it's been more than a day since last visit, add one to visit
        visits = visits + 1   
        request.session['last_visit'] = str(datetime.now())     # Update the last visit cookie now that we have updated the count
    else:
        request.session['last_visit'] = last_visit_cookie       # Set the last visit cookie
